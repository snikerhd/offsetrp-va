...
export default function App() {
  ...
  // Multiple selected infractions
  const [selectedInfractions, setSelectedInfractions] = useState<string[]>([]);
  
  // Advanced Fields
  const [agenteInfo, setAgenteInfo] = useState('');
  const [valorMultaManual, setValorMultaManual] = useState('');
  const [localidade, setLocalidade] = useState('');
  const [tempoApreensaoHoras, setTempoApreensaoHoras] = useState('24');
  ...
  const penalCodeDPLS = [
    { op: "1", competencia: "Competência DPLS", motivo: "Veículo apreendido num assalto", custos: "0", coima: "45000" },
    { op: "2", competencia: "Competência DPLS", motivo: "Veículo apreendido no decorar de tentativa de fuga às autoridades", custos: "0", coima: "25000" },
    { op: "3", competencia: "Competência DPLS", motivo: "Veículo apreendido por falta de carta de condução", custos: "0", coima: "15000" },
    { op: "4", competencia: "Competência DPLS", motivo: "Veículo apreendido no decorrar de uma investigação", custos: "0", coima: "0" },
    { op: "5", competencia: "Competência DPLS", motivo: "Veículo apreendido por utilização da viatura para causar danos ou desacatos de forma premeditada", custos: "0", coima: "10000" },
  ];

  const penalCodeDBC = [
    { op: "1", competencia: "Competência DBC", motivo: "Veículo apreendido num assalto", custos: "0", coima: "45000" },
    { op: "2", competencia: "Competência DBC", motivo: "Veículo apreendido no decorrar de tentativa de fuga às autoridades", custos: "0", coima: "25000" },
    { op: "3", competencia: "Competência DBC", motivo: "Veículo apreendido por falta de carta de condução", custos: "0", coima: "15000" },
    { op: "4", competencia: "Competência DBC", motivo: "Veículo apreendido no decorrar de uma investigação", custos: "0", coima: "0" },
    { op: "5", competencia: "Competência DBC", motivo: "Veículo apreendido por utilização da viatura para causar danos ou desacatos de forma premeditada", custos: "0", coima: "10000" },
    { op: "6", competencia: "Competência DBC", motivo: "Abandono de veículo na via pública", custos: "0", coima: "4500" },
    { op: "7", competencia: "Competência DBC", motivo: "Alterações estéticas ilegais no veículo", custos: "0", coima: "12000" },
    { op: "8", competencia: "Competência DBC", motivo: "Estacionar em local proibido", custos: "0", coima: "2500" },
  ];

  const getPenalCode = () => {
    switch (department) {
      case 'DPSA': return penalCodeDPSA;
      case 'DPLS': return penalCodeDPLS;
      case 'DBC': return penalCodeDBC;
      default: return penalCodeDPSA;
    }
  };

  const penalCode = getPenalCode();

  // Calculate total fine from selected infractions
  const calcularMultaTotal = () => {
    let total = 0;
    selectedInfractions.forEach(op => {
      const found = penalCode.find(p => p.op === op);
      if (found) {
        total += parseInt(found.coima);
      }
    });
    return total;
  };

  // Toggle infraction selection (checkbox behavior)
  const toggleInfraction = (op: string) => {
    playClick();
  ...
  // Get selected infractions details for report
  const getSelectedInfractionsDetails = () => {
    return selectedInfractions.map(op => {
      const found = penalCode.find(p => p.op === op);
      return found;
    }).filter(Boolean);
  };
  ...
  const handleGerarRelatorio = (e: React.FormEvent) => {
    ...
    if (selectedInfractions.length === 0 && !customMotive.trim()) {
      alert('Selecione pelo menos uma infração ou indique um motivo personalizado.');
      return;
    }
    ...
    const multaTotal = calcularMultaTotal();
    ...
    // Build infractions list text
    const infractionsDetails = getSelectedInfractionsDetails();
    let motivosTexto = '';
    
    if (infractionsDetails.length > 0) {
      motivosTexto = infractionsDetails.map((inf, idx) => 
        `${idx + 1}. ${inf!.motivo} (${parseInt(inf!.coima).toLocaleString('pt-PT')}€)`
      ).join('\n');
    }
    ...
    if (isAdvancedMode) {
      relatorio = (
        ...
        `COMPETÊNCIA JURÍDICA: ${department === 'DPSA' ? 'Competência DPSA' : department === 'DPLS' ? 'Competência DPLS' : 'Competência DBC'}\n\n` +
        `DOCUMENTO DO CONDUTOR (CC): ${numero_cc}\n\n` +
        `INFRAÇÕES SELECIONADAS:\n` +
        `   ${motivosTexto}\n` +
        (customMotive.trim() ? `   ${infractionsDetails.length + 1}. ${customMotive.trim()}\n` : '') +
        `\n` +
        `MULTA TOTAL: ${multaTotal.toLocaleString('pt-PT')}€\n` +
        (agenteInfo ? `REGISTADO POR: ${agenteInfo}\n` : '') +
        (valorMultaManual ? `VALOR COBRADO: ${parseInt(valorMultaManual).toLocaleString('pt-PT')}€\n` : '') +
    ...
    // Build full motive string for history
    const fullMotivo = [
      ...infractionsDetails.map(inf => inf!.motivo),
      customMotive.trim()
    ].filter(Boolean).join('; ');
    ...
    const newReport: SavedReport = {
      ...
      valorMulta: multaTotal.toString(),
      locality: localidade || undefined,
      competencia: department
    };
  ...
  const accentColor = department === 'DPSA' ? 'text-amber-400' : department === 'DPLS' ? 'text-blue-400' : 'text-emerald-400';
  const borderHighlight = department === 'DPSA' ? 'border-amber-500/35' : department === 'DPLS' ? 'border-blue-500/35' : 'border-emerald-500/35';
  const bgGradient = department === 'DPSA'
    ...
    : department === 'DPLS' 
      ...
      : 'from-emerald-950/95 via-slate-900/98 to-neutral-950/100';
  ...
  // Selected count and total for display
  const selectedCount = selectedInfractions.length;
  const multaTotal = calcularMultaTotal();
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${bgGradient} text-white font-sans flex flex-col justify-between relative selection:bg-white/20 overflow-x-hidden`}>
      ...
      <div className="absolute top-0 left-0 right-0 h-1.5 flex z-50">
        <div className={`flex-1 transition-all duration-1000 ${department === 'DPSA' ? 'bg-amber-600 animate-pulse' : department === 'DPLS' ? 'bg-blue-600 animate-pulse' : 'bg-emerald-600 animate-pulse'}`} />
        <div className="w-12 bg-white/20 animate-ping absolute left-1/2 transform -translate-x-1/2 h-1.5" />
        <div className={`flex-1 transition-all duration-1000 ${department === 'DPSA' ? 'bg-yellow-600 animate-pulse delay-500' : department === 'DPLS' ? 'bg-red-600 animate-pulse delay-500' : 'bg-teal-400 animate-pulse delay-500'}`} />
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs text-gray-400 font-mono flex items-center gap-1.5">
              ...
              <span className="text-emerald-400/90 font-bold">OFFSET PORTUGAL RP</span>
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col space-y-6">
          ...
          <div className={`bg-neutral-950/90 rounded-xl p-4 border border-white/10 ${neonShadow} flex flex-col items-center justify-center space-y-3 relative overflow-hidden`}>
            <div className="absolute top-2 left-2 flex items-center gap-1.5 text-[10px] font-mono font-bold text-gray-400">
              <span className={`inline-block w-2 h-2 rounded-full ${department === 'DPSA' ? 'bg-amber-500' : department === 'DPLS' ? 'bg-blue-500' : 'bg-emerald-500'}`} />
          ...
          <div className="bg-slate-900/60 backdrop-blur-md rounded-xl p-5 border border-white/5 flex flex-col justify-between space-y-4">
            <form onSubmit={handleGerarRelatorio} className="space-y-4">
              ...
              {/* INFRACTIONS SELECTION (CHECKBOXES - MULTI SELECT) */}
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="block text-[11px] font-bold text-gray-300 uppercase font-mono">
                    Selecionar Infrações
                    <span className="ml-2 text-emerald-500 normal-case font-normal">({department})</span>
                  </label>
                  {selectedCount > 0 && (
                ...
                <div className="space-y-1.5 max-h-[180px] overflow-y-auto pr-1 custom-scrollbar">
                  {penalCode.map((item) => (
                    <label
                      key={item.op}
                      onClick={() => toggleInfraction(item.op)}
                      className={`flex items-start gap-2.5 p-2.5 rounded-lg border text-xs font-semibold cursor-pointer transition-all duration-200 ${
                        selectedInfractions.includes(item.op)
                          ? department === 'DPSA' ? 'bg-amber-600/20 border-amber-500 text-amber-100' :
                            department === 'DPLS' ? 'bg-blue-600/20 border-blue-500 text-blue-100' :
                    ...
                    >
                      <div className={`w-4 h-4 mt-0.5 rounded flex items-center justify-center border-2 ${
                        selectedInfractions.includes(item.op)
                          ? department === 'DPSA' ? 'bg-amber-500 border-amber-500' :
                            department === 'DPLS' ? 'bg-blue-500 border-blue-500' :
                      ...
                      <div className="flex-1 font-mono text-[11px] leading-tight">
                        ...
                        <span className="flex items-center justify-between">
                          ...
                          <span className={`text-[10px] font-bold ml-2 ${department === 'DBC' ? 'text-emerald-400' : 'text-yellow-400'}`}>
                ...
                {/* MULTA TOTAL Display */}
                {selectedCount > 0 && (
                  <div className={`mt-3 p-3 rounded-lg border-2 ${department === 'DPSA' ? 'border-amber-500/50 bg-amber-500/10' : department === 'DPLS' ? 'border-blue-500/50 bg-blue-500/10' : 'border-emerald-500/50 bg-emerald-500/10'}`}>
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-bold uppercase font-mono text-gray-300">Multa Total:</span>
                      <span className={`text-lg font-extrabold font-mono ${department === 'DPSA' ? 'text-amber-400' : department === 'DPLS' ? 'text-blue-400' : 'text-emerald-400'}`}>
                        {multaTotal.toLocaleString('pt-PT')}€
                      </span>
                    </div>
                    <div className="text-[10px] text-gray-400 mt-1 font-mono">
                      {selectedCount} infração{selectedCount > 1 ? 'ões' : ''} selecionada{selectedCount > 1 ? 's' : ''}
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Custom Motive */}
              <div className="space-y-1">
                <label className="block text-[11px] font-bold text-gray-400 uppercase font-mono">Motivo Personalizado (Opcional)</label>
                <textarea
                  placeholder="Adicione um motivo extra se necessário..."
                  value={customMotive}
                  onChange={(e) => {
                    playClick();
                  ...
                  rows={2}
                  className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-white/30 font-mono placeholder-gray-600"
                />
              </div>
              ...
              {isAdvancedMode && (
                <div className="space-y-3 bg-black/40 p-3 rounded-lg border border-white/10 animate-fadeIn text-xs font-mono">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="text"
                      placeholder="Ex: Agente Silva (DBC-42)"
                      value={agenteInfo}
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="text"
                      placeholder="Ex: Sandy Shores / Paleto Bay"
                      value={localidade}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      <label className="block text-[10px] font-bold text-gray-400 uppercase">Valor Cobrado (€)</label>
                      <input
                        type="number"
                        placeholder="Opcional - Override"
                        value={valorMultaManual}
                        onChange={(e) => { playClick(); setValorMultaManual(e.target.value); }}
                        className="w-full bg-slate-900 border border-white/10 rounded px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-white/20 font-mono text-emerald-400 font-bold"
                      />
                    </div>
        ...
        <div className="lg:col-span-7 flex flex-col space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs uppercase font-extrabold tracking-wider text-emerald-500 font-mono">
              ...
              {generatedReportText ? (
                <div className="flex-1 flex flex-col justify-between space-y-3">
                  ...
                  <p className="text-[10px] text-gray-500 font-mono leading-relaxed flex items-start gap-1.5">
                    ...
                    <span>DICA: O editor acima é interativo. Pode clicar no texto para o editar manualmente ou adicionar observações adicionais antes de copiar para as plataformas de logging.</span>
                  </p>
                </div>
              ) : (
                <div className="flex-1 flex flex-col items-center justify-center text-center py-16 space-y-4">
                  <div className={`p-4 rounded-full bg-neutral-900 border border-white/5 animate-pulse ${department === 'DPSA' ? 'text-amber-500/20' : department === 'DPLS' ? 'text-blue-500/20' : 'text-emerald-500/20'}`}>
          ...
          <div className="bg-slate-900/40 border border-white/5 rounded-xl p-4 font-mono text-xs space-y-3">
              ...
              <table className="w-full text-left border-collapse text-[11px]">
                <thead>
                  <tr className="bg-neutral-900 text-gray-300 border-b border-white/10">
                    <th className="p-2 font-bold uppercase">#</th>
                    ...
                    >
                      <td className="p-2 text-center">
                        <div className={`w-5 h-5 rounded flex items-center justify-center mx-auto ${
                          selectedInfractions.includes(item.op)
                            ? department === 'DPSA' ? 'bg-amber-500 text-white' :
                              department === 'DPLS' ? 'bg-blue-500 text-white' :
                              'bg-emerald-500 text-white'
                            : 'border border-gray-500'
                        }`}>
                          {selectedInfractions.includes(item.op) && <Check className="w-3 h-3" />}
                        </div>
                      </td>
            ...
            <p className="text-[10px] text-gray-500 text-center italic font-sans leading-snug">
              Clique numa linha da tabela para selecionar/desmarcar. Pode selecionar múltiplas infrações.
            </p>
          </div>
        </div>
      </main>
                  ...
                  return (
                    <div 
                      ...
                      className={`group p-3 rounded-lg border bg-black/50 hover:bg-black/80 transition-all duration-200 cursor-pointer text-xs flex flex-col justify-between space-y-2 relative ${
                        isCurrentlyActive 
                          ? `${borderHighlight} bg-neutral-900/60 shadow-[0_0_8px_rgba(16,185,129,0.1)]` 
                          : 'border-white/5'
                      }`}
                    >
                      <div className="space-y-1">
                        <div className="flex justify-between items-center text-[9px] font-mono text-gray-500">
                          <span className={`px-1.5 py-0.5 rounded ${isCurrentlyActive ? 'bg-emerald-400/10 text-emerald-400 font-bold' : 'bg-neutral-800 text-gray-400'}`}>
                            {rep.id}
                          </span>
      ...
      <footer className="border-t border-white/5 bg-slate-950 py-3 text-center text-[10px] sm:text-xs text-gray-500 font-mono z-10 relative">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-2">
          ...
          <span className="opacity-60 flex items-center gap-1">
            <span>© {systemTime.getFullYear()} {department}</span>
            <span>&bull;</span>
            <span className="text-emerald-400">Codificação Segura UTF-8</span>
          </span>
        </div>
      </footer>
